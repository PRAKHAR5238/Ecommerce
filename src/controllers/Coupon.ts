...
export const newCoupon = TryCatch(
  async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!code || !discountValue || !discountType || !expirationDate) {
      return next(
        new ErrorHandler(
          "Please provide all required fields: code, discountValue, discountType, expirationDate, and createdBy.",
          400
        )
      );
    }
    ...
    if (existingCoupon) {
      return next(
        new ErrorHandler("A coupon with this code already exists.", 409)
      );
    }
